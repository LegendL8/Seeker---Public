...
describe('fetchApplicationsList', () => {
  ...
  it('throws with API message on non-ok response', async () => {
    ...
    await expect(fetchApplicationsList(1, 20)).rejects.toThrow('Not authenticated');
  });
...
describe('createApplication', () => {
  ...
  it('throws with API message on non-ok response', async () => {
    ...
    ).rejects.toThrow('jobTitle is required');
  });
});
...
describe('updateApplication', () => {
  it('PATCHes body to correct URL and returns application', async () => {
    ...
    const updated = { id: 'uuid-1', jobTitle: 'Senior Engineer', status: 'interviewing' };
    mockFetch.mockResolvedValueOnce({
      ok: true,
