...
export async function fetchResumeWithUrl(id: string): Promise<ResumeWithSignedUrl> {
  const base = getApiBaseUrl();
...
export async function setResumeActive(id: string, isActive: boolean): Promise<Resume> {
  const base = getApiBaseUrl();
  const res = await fetch(`${base}/v1/resumes/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    credentials: 'include',
