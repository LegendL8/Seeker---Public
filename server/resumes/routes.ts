...
import { createResume, deleteResume, getResumeWithSignedUrl, listResumes, setActiveResume } from './service';
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!file || !file.buffer) {
      throw new ValidationError('No file uploaded. Send multipart/form-data with field "file" (PDF or DOCX).');
    }
    const fileType = mimeToFileType(file.mimetype);
    if (!fileType) {
      throw new ValidationError('Invalid file type. Only PDF and DOCX are allowed.');
    }
    const fileName = file.originalname?.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 255) || `resume.${fileType}`;
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const row = await setActiveResume(user.id, idResult.data, bodyResult.data.isActive);
    res.json({
      data: {
        id: row.id,
        fileName: row.fileName,
