...
jest.mock('./auth/middleware', () => ({
  requireAuth: (req: express.Request, res: express.Response, next: () => void) => {
    if (req.user) next();
    else res.status(401).json({ error: 'UNAUTHORIZED', message: 'Not authenticated', statusCode: 401 });
  },
}));
...
describe('GET /api/v1/dashboard/metrics', () => {
  ...
  it('returns 200 and metrics in data when authenticated', async () => {
    const res = await fetch(`http://127.0.0.1:${port}/api/v1/dashboard/metrics`);
