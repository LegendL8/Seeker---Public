...
test("notes list loads", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Notes", level: 2 })).toBeVisible({ timeout: 10000 });
...
test("note detail and edit with debounced save", async ({ page }) => {
  await page.goto("/notes");
  await expect(page.getByRole("heading", { name: "Notes", level: 2 })).toBeVisible({ timeout: 10000 });
  const firstNote = page.getByRole("button", { name: /general|interview|research|job description/i }).first();
  ...
  await expect(page.getByText("Saving…").or(page.getByText("Saved"))).toBeVisible({ timeout: 3000 });
...
test("add note", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "Content" }).fill("E2E new note " + Date.now());
...
test("delete note", async ({ page }) => {
  await page.goto("/notes");
  await page.getByRole("button", { name: "Add note" }).first().click();
  await page.getByRole("textbox", { name: "Content" }).fill("E2E note to delete");
  await page.locator("form").getByRole("button", { name: "Add note" }).click();
  await expect(page.getByText("E2E note to delete")).toBeVisible({ timeout: 5000 });
  await page.getByRole("button", { name: /E2E note to delete/i }).click();
  await expect(page.getByRole("button", { name: "Delete note" })).toBeVisible({ timeout: 3000 });
  page.on("dialog", (d) => d.accept());
  await page.getByRole("button", { name: "Delete note" }).click();
  await expect(page.getByText("Select a note or add a new one.")).toBeVisible({ timeout: 5000 });
});
